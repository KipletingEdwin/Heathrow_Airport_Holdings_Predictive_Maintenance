...
# =====================================================================
# FILE 2: data_pipeline.py
# Purpose: Handles Ingestion, Feature Selection, and Preprocessing 
#          (Steps 1, 2, and 3 of the Checklist)
# =====================================================================

def load_and_inspect_data(file_path="ai4i2020.csv"):
    """
    Step 1: Loads the raw telemetry matrix and inspects structural dimensions.
    ...
    # Verify shape parameters (Expects 10,000 rows, 14 columns)
    rows, cols = df.shape
    print(f" -> [STATUS] Dataset loaded successfully. Shape: {rows} rows, {cols} columns.")
    
    # Programmatic check for missing values across all telemetry features
    null_counts = df.isnull().sum().sum()
    print(f" -> [STATUS] Integrity Check: Identified {null_counts} total missing/null entries.")
...
def clean_and_select_features(df):
    ...
    print(f" -> [STATUS] Isolated binary target variable 'Machine failure'. Failure Rate: {round(y.mean()*100, 2)}%")
    ...
    print(f" -> [STATUS] Dropped unique identifiers and target leakage columns: {columns_to_drop[2:]}")
    print(f" -> [STATUS] Extracted raw feature matrix shape: {X_raw.shape}")
...
def preprocess_and_scale_features(X_raw):
    ...
    print(" -> [STATUS] Applied One-Hot Encoding to categorical asset 'Type'.")
    print(f" -> [STATUS] Expanded columns check: {list(X_encoded.columns)}")
    
    # 3.3 Apply Feature Scaling (Standard Scaling) to all continuous operational variables
    print(" -> [STATUS] Initializing StandardScaler to balance multi-sensor metric ranges...")
...
def run_data_pipeline_module(file_path="ai4i2020.csv"):
    ...
    """
    print("="*60)
    print("[PIPELINE BLOCK] Starting Preprocessing and Data Refinery Phase...")
    print("="*60)
